...
public class FrodoKEMImpl extends NativeResource {
    ...
    public FrodoKEMImpl(String parameterSet) {
        ...
        byte[][] keyPair = null;
        try {
            keyPair = CryptoNative.frodoKemGenerateKeyPair(nativeContext, this.parameterSet);
            if (keyPair == null || keyPair.length != 2 || keyPair[0] == null || keyPair[1] == null) {
                throw new IllegalStateException("Generated FrodoKEM key pair is invalid");
            }
    ...
    void setKeys(byte[] encapKey, byte[] decapKey) {
        ...
        try {
            CryptoNative.frodoKemSetKeys(nativeContext, keyMaterial.publicKey(), keyMaterial.privateKey());
            updated = true;
